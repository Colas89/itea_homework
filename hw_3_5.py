...
print('\nСейчас мы наполним словарь, словами для перевода, в конце мы увидим наш перевод'
      '\n------------------------------------------------------'
      '\n')
...
def get_vocabluary(text):  # перенос ввода в словарь для дальнейшего перевода
    ...
    def text_change(text):  #функция возвращает список слов
        text = text.lower()
        for char in whitespace:  #функция убирает лишние символы
            if char in text:
                text = text.replace(char, " ")
        for char in punctuation:  #функция убирает специальные символы
            if char in text:
                text = text.replace(char, " ")
        text = text.split(" ")
    ...
    def trans_text(text, vocabr):  # функция переводит вводимый текст
        ...
        text = text.lower()
        for char in point:
            if char in text: text = text.replace(char, " . ")  #для красоты
        for char in comma:
            if char in text: text = text.replace(char, " , ")  #для красоты
...
if __name__ == "__main__":
    ...
    while text != "" and n == 0:
        ...
        vocabluary1.pop(".")  # удаляем из словаря точку (для красоты)
        vocabluary1.pop(",")  # удаляем из словаря пробелл (для красоты)
        print("\n")
